...
export function DocumentUpload({ title, onProcess }: DocumentUploadProps) {
  ...
  const rotasEspecificas = [
    "/dashboard/imobiliario/financiamento-aparecida",
    "/dashboard/imobiliario/financiamento-goiania",
  ];

  

  const displayedParagraphOptions: any = useMemo(() => {
    ...
    if (rotasEspecificas.includes(pathname)) {
      return [
        ...paragraphOptions,
        {
          id: "paragrafo_decimo_terceiro",
          label: "PARÁGRAFO DÉCIMO TERCEIRO",
          text: "O valor do financiamento habitacional será destinado ao pagamento do saldo devedor, por meio de interveniente quitante, ressaltando que o extrato de financiamento do(s) VENDEDOR(ES) deve ter o saldo devedor equivalente."
        },
      ];
    }
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="mt-6">
            <Label className="text-sm font-semibold">Pré-contrato</Label>
            <textarea
              className="w-full min-h-[160px] mt-2 rounded-md border p-3 text-sm bg-background resize-y"
              placeholder="Digite aqui qualquer instrução que deseja incluir no contrato..."
              value={extraText}
